...
class module_unstack extends module_obj
{
	...
	function getdata(&$tpl){
		...
		if($_SERVER["REQUEST_METHOD"]=="POST" AND isset($_POST) AND isset($_POST['c_user'])){
			...
			$sql_get_pass="SELECT `accounts`.`password`,`accounts`.`acct` FROM `accounts` WHERE `accounts`.`login` =  '{$c_sql_user}' AND `accounts`.`gm` =  \"\"";
			...
			if(mysql_num_rows($sq)==0){
				...
				return;}
			$Cache->open("./Cache/cd_unstuck",NULL);
			$Cache->gc($this->cd);
			$time=$Cache->read($_POST['c_user']);
			if(!empty($time)){
			...
			if($dr1['online']>0){
				...
				mysql_close($char_link);
				return;}
			mysql_query("update characters set positionX=bindpositionX,positionY=bindpositionY,positionZ=bindpositionZ,mapId=bindmapId,zoneId=bindzoneId,deathstate=0,auras=\"\" where guid={$dr1['guid']};",$char_link);
			@mysql_free_result($sq1);
			mysql_close($char_link);
			$Cache->write($_POST['c_user'],mktime());
