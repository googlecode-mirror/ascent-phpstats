<?php
/*
 * Project Name: Online Players Map for Antrix Server
 * Date: 11.06.2007 
 * Author: unknown (thanks to mirage666 for the original idea)
 * and (thanks to Q.SA for the develop in full)
...
function get_realm_name(){
 global $realm_db, $antrix_db;

 mysql_connect($realm_db['addr'], $realm_db['user'], $realm_db['pass']);
 mysql_select_db($realm_db['name']);

 $result = mysql_query("SELECT name FROM `realmlist` WHERE id = '{$antrix_db['id']}'");
